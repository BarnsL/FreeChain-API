...
export function applyMinorRepair({ root, file, search, replace, allowFiles }) {
  ...
  // so on Windows this must go through the shell or it fails with EINVAL.
  const tests = syntax.status === 0 ? spawnSync(process.platform === 'win32' ? 'npm.cmd' : 'npm', ['test'], { cwd: root, encoding: 'utf8', timeout: 120_000, shell: process.platform === 'win32' }) : syntax;
