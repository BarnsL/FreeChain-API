...
const safeRecord = (record = {}) => {
  ...
  const attempts = Array.isArray(record.attempts) ? record.attempts.slice(0, 100).map((attempt) => compact({
    ...
    providerStatus: finiteNumber(attempt?.providerStatus) ?? finiteNumber(String(attempt?.detail || '').match(/\b(\d{3})\b/)?.[1]),
  })) : undefined;
  ...
  return compact({
    ...
    localKeyName: cleanText(record.localKeyName, 120),
...
export class RequestJournal {
  ...
  query({ limit = 50, before, status, provider, app, route, harness, q, ownerId } = {}) {
