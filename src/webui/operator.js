// Chat page for the FreeChain dashboard.
//
// Kept deliberately in step with SubChain's src/webui/operator.js — the two
// dashboards share this surface, so a fix to one belongs in the other. The
// only intended divergence is the log-policy control below, which exists
// because FreeChain defaults prompt-summary retention to off.
//
...
async function opRefreshContext() {
  ...
  // The status line reports what just happened, not what the app is.
...
async function opLoadSettings() {
  ...
  for (const [id, key] of OP_LOG_FLAGS) opEl(id).checked = Boolean(settings.logs[key]);
  opEl('opMaxRawChars').value = settings.logs.maxRawChars ?? 20000;
}
...
// FreeChain-only. Prompt summaries are bounded and redacted, but they are still
// user content, so retention is opt-in rather than a silent default.
opEl('opSaveLogs').addEventListener('click', async () => {
  ...
  // Storing live bearer tokens is the one switch here that can burn something
  ...
  if (logs.credentials && !opEl('opCredentials').dataset.confirmed) {
    if (!confirm(
      ...
      + 'Every bearer token a caller sends will be written to logs/requests.jsonl '
      + 'unredacted. Anyone who can read that file gets working keys.\n\n'
      + 'Only do this to diagnose a rejected key, then turn it off and clear the journal.',
    )) {
  ...
  try {
    ...
    await opLoadSettings();
    // The Logs page states the policy in force, so it has to be corrected now
    // rather than on the next reload: a stale "never stored" line is exactly
    // the claim that notice exists to prevent.
    renderRetentionNotice(logs);
