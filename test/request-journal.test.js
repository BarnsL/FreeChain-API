...
test('request and response summaries count content without retaining it', () => {
  ...
  assert.deepEqual(input, {
    ...
    maxTokens: 321,
    promptSummary: [
      { role: 'system', summary: 'private system text' },
      { role: 'user', summary: 'private user text' },
    ],
  });
  ...
  assert.deepEqual(input.promptSummary.map((item) => item.role), ['system', 'user']);
  assert.doesNotMatch(JSON.stringify(output), /private|secret/i);
});
...
test('raw fields survive the safeRecord allowlist and clear when switched off', () => {
  ...
  journal.append({
    ...
    auth: { result: 'rejected', ownership: 'freechain-access-key', credential: 'Bearer CRED_SENTINEL' },
  });
  ...
  journal.append({
    ...
    auth: { result: 'accepted', ownership: 'freechain-access-key' },
  });
