...
A minor repair is last resort. It is one exact search/replace within an allowlisted UI/operator file, limited in size. The server creates a backup, runs `node --check` where applicable and the full `npm test`; a failure restores the original automatically. Security/auth/control code is intentionally blocked.
