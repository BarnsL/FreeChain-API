...
FreeChain imports third-party prompt collections into private platform
...
| Source | Imported paths | Use in FreeChain |
|---|---|---|
...
Run `npm run import-presets` to fetch current revisions into
`%APPDATA%\FreeChain\presets` (or the platform equivalent; `FREECHAIN_DATA_DIR`
overrides it). Each private source
...
6. Press **Make active** if that Harness should compose live requests. FreeChain
   applies exactly one Harness at a time — see [HARNESS.md](HARNESS.md).
